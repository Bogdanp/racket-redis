...
;; DEL key [key ...]
(define-variadic-command (remove! [key redis-key/c] . [keys redis-key/c])
  #:command-name "DEL"
  #:result-contract exact-nonnegative-integer?)
...
;; LINDEX key index
(define-simple-command (list-ref [key redis-key/c] [index exact-integer? #:converter number->string])
  #:command-name "LINDEX"
  #:result-contract redis-value/c)
